...
(defvar *bulk-idempotency-db-lock* (bt:make-lock "bulk-idempotency-db"))
...
(defun sha256-hex (string)
  (let ((digest
    ...
    (with-output-to-string (stream)
      (map nil (lambda (octet) (format stream "~2,'0x" octet)) digest))))
...
(defun canonicalize-json-value (value)
  "Canonicalize object-key order while preserving array order."
  (cond
    ((json-object-p value)
     (let ((keys nil)
...
(defun bulk-request-fingerprint (documents)
  (sha256-hex
   (jsown:to-json (mapcar #'canonicalize-json-value documents))))
...
(defun idempotency-job-id (principal-id tenants key)
  (format nil "idem-~a"
          (sha256-hex
           (format nil "~a~%~{~a~^,~}~%~a" principal-id tenants key))))
...
(defun bulk-idempotency-record-id (job-id)
        ...
        (anypool:with-connection (client *couchdb-pool*)
          (handler-case
            ...
            (dexador:http-request-not-found ()
              (ignore-errors
                (cl-couch:create-database client *bulk-idempotency-database*))
              (cl-couch:get-database client *bulk-idempotency-database*)))
        ...
        (let* ((response
                 (jsown:parse
                  (cl-couch:create-document
                   client *bulk-idempotency-database* (jsown:to-json record))))
...
(defvar *bulk-idempotency-load-fn* #'couch-load-bulk-idempotency-record)
(defvar *bulk-idempotency-save-fn* #'couch-save-bulk-idempotency-record)
...
(defun bulk-idempotency-record-expired-p (record now)
  (let ((expires-at (jsown:val-safe record "expires_at")))
    (and (integerp expires-at) (<= expires-at now))))

(defun bulk-idempotency-reclaimable-p (record now)
  "Only known-finished records may be reused after TTL expiry.
Unresolved/indeterminate reservations stay fail-closed indefinitely."
  (and (bulk-idempotency-record-expired-p record now)
       (member (or (jsown:val-safe record "status") "reserved")
               '("completed" "completed-with-errors" "rejected")
               :test #'string=)))
...
(defun reserve-bulk-idempotency
  ...
  "Atomically reserve KEY before any Rabbit publication."
  (let* ((tenants (bulk-request-tenants documents))
         ...
         (record-id (bulk-idempotency-record-id job-id)))
    (handler-case
        (loop for attempt from 1 to max-attempts
              ...
              do
                 (cond
                   ((or (null existing)
                        (bulk-idempotency-reclaimable-p existing now))
                    (let ((candidate
                      ...
                      (handler-case
                          (return
                            (values (save-bulk-idempotency-record candidate)
                                    :owner))
                        (bulk-idempotency-store-conflict ()
                          (when (= attempt max-attempts)
                            (error "Bulk idempotency reservation conflict budget exhausted"))))))
                   ((string= fingerprint
...
(defun update-bulk-idempotency-record
    (job-id updater &key (max-attempts 8))
  (let ((record-id (bulk-idempotency-record-id job-id)))
    (loop for attempt from 1 to max-attempts
          ...
          do
             ...
             (let ((updated (funcall updater (clone-json-value current))))
               (handler-case
                 ...
                 (bulk-idempotency-store-conflict ()
                   (when (= attempt max-attempts)
                     (error "Bulk idempotency update conflict budget exhausted"))))))))

(defun bulk-status-rank (status)
  (cond
    ((string= status "reserved") 0)
    ((string= status "accepted") 1)
    ((string= status "running") 2)
    (t 3)))

(defun bulk-terminal-status-p (status)
  (member status
          '("completed" "completed-with-errors" "failed" "indeterminate" "rejected")
          :test #'string=))
  ...
  (update-bulk-idempotency-record
   ...
   (lambda (record)
     (let ((current (or (jsown:val-safe record "status") "reserved")))
       (when (or (string= current status)
                 (and (not (bulk-terminal-status-p current))
                      (>= (bulk-status-rank status)
                          (bulk-status-rank current))))
         (setf (jsown:val record "status") status)
         (when (integerp succeeded)
           (setf (jsown:val record "succeeded") succeeded))
         (when (integerp failed)
           (setf (jsown:val record "failed") failed))
         (when error-code
           (setf (jsown:val record "error_code") error-code))))
     record)))

(defun store-bulk-idempotency-initial-response (job-id status-code body)
  (update-bulk-idempotency-record
   job-id
   (lambda (record)
...
(defun reject-bulk-idempotency-reservation (job-id)
  "Release a reservation only when rejection is known to precede enqueue/publish."
  (update-bulk-idempotency-record
   job-id
   (lambda (record)
     (when (string= (or (jsown:val-safe record "status") "reserved")
                    "reserved")
       (setf (jsown:val record "status") "rejected"
             (jsown:val record "expires_at") (bulk-idempotency-now)))
     record)))
        ...
        (progn
          ...
          (jsown:to-json
           (jsown:new-js
             ...
             ("total" (or (jsown:val-safe record "total") 0))
             ("status_url"
              (bulk-idempotency-status-url (jsown:val record "job_id")))
             ("correlation_id" (jsown:val-safe record "correlation_id"))
             ("idempotent_replay" :true)))))))

(defun load-bulk-idempotency-by-job-id (job-id)
  (when (idempotency-job-id-p job-id)
    (load-bulk-idempotency-record (bulk-idempotency-record-id job-id))))
