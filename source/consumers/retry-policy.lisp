...
(defun quarantine-record (stream settlement)
  (let* ((properties (retry-stream-current-properties stream))
    ...
    (setf (jsown:val record "_id")
          ...
          (or (retry-stream-current-received-at stream) (star.documents:utc-now))
...
(defmethod make-rabbit-worker-consumer ((consumer retrying-rabbit-consumer) worker-number)
