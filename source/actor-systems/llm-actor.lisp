;;;; llm.starintel.actor -- dataset synthesis and fine-tune orchestration
...
(defun llm-job-update (job &key status provider offer-id instance-id hourly-rate
  ...
  (bt:with-lock-held (*llm-fine-tune-jobs-lock*)
    (when status (setf (llm-fine-tune-job-status job) status))
    (when provider (setf (llm-fine-tune-job-provider job) provider))
    (when offer-id (setf (llm-fine-tune-job-offer-id job) offer-id))
    (when instance-id (setf (llm-fine-tune-job-instance-id job) instance-id))
    (when hourly-rate (setf (llm-fine-tune-job-hourly-rate job) hourly-rate))
    (when work-directory (setf (llm-fine-tune-job-work-directory job) work-directory))
    (when output-directory (setf (llm-fine-tune-job-output-directory job) output-directory))
    (when error (setf (llm-fine-tune-job-error job) error))
...
(defun llm-job-json-object (job)
  (let ((json (jsown:new-js
    ...
    (when (llm-fine-tune-job-provider job)
      (setf (jsown:val json "provider") (llm-fine-tune-job-provider job)))
    (when (llm-fine-tune-job-offer-id job)
      (setf (jsown:val json "offer_id") (llm-fine-tune-job-offer-id job)))
    (when (llm-fine-tune-job-instance-id job)
      (setf (jsown:val json "instance_id") (llm-fine-tune-job-instance-id job)))
    (when (llm-fine-tune-job-hourly-rate job)
      (setf (jsown:val json "hourly_rate_usd") (llm-fine-tune-job-hourly-rate job)))
    ...
    (when (llm-fine-tune-job-error job)
      (setf (jsown:val json "error") (llm-fine-tune-job-error job)))
    json))
...
(defun llm-synthetic-candidate-valid-p (candidate)
  (let ((messages (and (listp candidate)
  ...
  (let* ((material (format nil "~a|~a|~a"
         ...
         (row
           (jsown:new-js
             ...
             ("metadata" (or (jsown:val-safe candidate "metadata")
                             (jsown:empty-object)))))))
    (when (jsown:val-safe candidate "tools")
      (setf (jsown:val row "tools") (jsown:val candidate "tools")))
    row))
...
(defun llm-synthesize-jsonl (request)
  ...
  (let* ((requested (or (jsown:val-safe request "max_examples") 0))
         ...
         (families (llm-sequence-list (jsown:val-safe request "task_families")))
         (batch-size (max 1 (llm-env-integer "STAR_LLM_SYNTH_BATCH_SIZE"
    ...
    (with-output-to-string (out)
      (loop with emitted = 0
            ...
            do (multiple-value-bind (teacher-json model)
                 ...
                 (let ((examples (llm-sequence-list
                   ...
                   (dolist (candidate examples)
                     (write-string
                      (jsown:to-json
                       (llm-decorate-synthetic-candidate candidate family model))
                      out)
                     (terpri out)
                     (incf emitted))))
               (incf family-index)))))
...
(defun llm-github-artifact-id (artifact)
  (let* ((repository (jsown:val artifact "repository"))
         ...
         (matches (llm-sequence-list (jsown:val-safe json "artifacts")))
         (match (find name matches
                      :test #'string=
                      :key (lambda (item) (jsown:val-safe item "name")))))
  ...
  (let* ((body
           (llm-run-program
            (list "vastai" "--raw" "create" "instance"
                  ...
                  "--disk" (princ-to-string +llm-default-gpu-disk-gb+)
                  "--ssh" "--direct"
...
(defun llm-vast-wait-running (instance-id)
  (loop with deadline = (+ (get-universal-time)
        ...
        do (cond
             ((string= (or status "") "running") (return state))
             ((member status '("exited" "unknown" "offline") :test #'string=)
              (error "Vast instance ~a entered terminal state ~a"
                     instance-id status))
             ((> (get-universal-time) deadline)
              (error "Timed out waiting for Vast instance ~a" instance-id)))
           (sleep 5)))
...
(defun llm-vast-execute (instance-id command)
  (llm-run-program
   (list "vastai" "execute" (princ-to-string instance-id) command)))
...
(defun llm-training-command (base-model max-seq-length budget-seconds)
  (format nil
          "set -euo pipefail; cd /workspace/star-model; python -m pip install --upgrade pip; python -m pip install -r trainer/requirements.txt; timeout --signal=TERM ~ds python trainer/train_qlora.py --base-model ~a --train train.jsonl --validation validation.jsonl --output /workspace/output --max-seq-length ~d"
  ...
  (let ((instance-id nil))
    (handler-case
        (multiple-value-bind (base-model max-seq-length gpu-ram max-hourly max-total artifact)
            (llm-fine-tune-request-values (llm-fine-tune-job-request job))
          (let* ((job-id (llm-fine-tune-job-id job))
                 (root (pathname
                        (or (uiop:getenv "STAR_LLM_JOB_ROOT")
                            "/var/tmp/starintel-llm/")))
                 (work-dir (merge-pathnames (format nil "~a/" job-id) root))
                 (archive (merge-pathnames "dataset.zip" work-dir))
                 (dataset-dir (merge-pathnames "dataset/" work-dir))
                 (output-dir (merge-pathnames "output/" work-dir)))
            (ensure-directories-exist (merge-pathnames "sentinel" work-dir))
            (llm-job-update job :status :fetching-dataset
                                :work-directory (namestring work-dir))
            (llm-download-github-artifact artifact archive work-dir)
            (ensure-directories-exist (merge-pathnames "sentinel" dataset-dir))
            (llm-run-program
             (list "unzip" "-q" (namestring archive)
                   "-d" (namestring dataset-dir)))
            (llm-job-update job :status :selecting-gpu :provider "vast-ai")
            (let* ((offers (llm-vast-search-offers gpu-ram max-hourly))
                   (offer (or (first offers)
                              (error "No Vast offers matched the configured budget")))
                   (offer-id (llm-vast-offer-id offer))
                   (hourly (llm-vast-hourly-rate offer))
                   (budget-seconds (llm-budget-seconds hourly max-total)))
              (when (> hourly max-hourly)
                (error "Selected Vast offer exceeds hourly budget"))
              (llm-job-update job :status :provisioning
                                  :offer-id offer-id
                                  :hourly-rate hourly)
              (setf instance-id (llm-vast-create-instance offer-id job-id))
              (llm-job-update job :instance-id instance-id)
              (llm-vast-wait-running instance-id)
              (llm-job-update job :status :staging)
              (llm-vast-copy-to-instance dataset-dir instance-id
                                         "/workspace/star-model/")
              (llm-job-update job :status :training)
              (llm-vast-execute
               instance-id
               (llm-training-command base-model max-seq-length budget-seconds))
              (llm-job-update job :status :collecting)
              (llm-vast-copy-from-instance instance-id "/workspace/output/" output-dir)
              (llm-job-update job :status :succeeded
                                  :output-directory (namestring output-dir)))))
      (error (condition)
        (log:error "llm fine-tune job ~a failed: ~a"
                   (llm-fine-tune-job-id job) condition)
        (llm-job-update job :status :failed :error (princ-to-string condition))))
    (llm-vast-destroy instance-id)
    job))

(defun llm-submit-fine-tune (request)
  "Queue a fine-tune job. The heavy work runs outside the actor dispatcher."
  ;; Validate synchronously so malformed requests fail before a worker is started.
  (multiple-value-call #'values (llm-fine-tune-request-values request))
...
(defun llm-dispatch-message (message)
  (case (getf message :op)
    (:synthesize
     (llm-synthesize-jsonl (getf message :request)))
    (:fine-tune
