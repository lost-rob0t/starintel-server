...
(defun ensure-test-database ()
  ...
  (handler-case
      (progn
        ...
        (setf star.databases.couchdb::*couchdb-default-database* *test-database*)
        ;; Use init-db to create database and views
...
(defclass stub-producer ()
  ()
  (:documentation "Stub producer for tests that doesn't connect to RabbitMQ"))

(defmethod star.producers:publish ((producer stub-producer) &key body routing-key properties)
  "Stub publish method that does nothing"
  (declare (ignore body routing-key properties))
  (dbg "Stub publish called (no-op)"))

(defun start-test-actor-system ()
  "Create stub actors for tests (without actually starting the actor system)."
  (dbg "Creating stub actors for tests")

  ;; Create a minimal actor system just for agent creation
  (setf star.actors:*sys* (sento.actor-system:make-actor-system))

  ;; Create a stub producer agent that returns a stub producer with publish method
  (setf star.actors:*producer-agent*
        (sento.agent:make-agent (lambda ()
                                  (make-instance 'stub-producer))
                                star.actors:*sys*))

  (dbg "Stub actors created"))

(defun stop-test-actor-system ()
  "Stop the test actor system."
  (dbg "Stopping actor system")
  (when star.actors:*sys*
    (handler-case
        (sento.actor-context:shutdown star.actors:*sys*)
      (error (e)
        (dbg "Error shutting down actor system: ~a" e)))
    (setf star.actors:*sys* nil)
    (setf star.actors:*producer-agent* nil))
  ;; Kill any remaining threads
  (loop for thread in (bt:all-threads)
        when (and (not (eq thread (bt:current-thread)))
                  (search "hunchentoot" (bt:thread-name thread) :test #'char-equal))
          do (progn
               (dbg "Killing thread: ~a" (bt:thread-name thread))
               (ignore-errors (bt:destroy-thread thread))))
  (dbg "Actor system stopped"))
