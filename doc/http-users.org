...
* Administrator user scope/status update

Administrators may update persisted authorization scopes or disable/reactivate a
human user without changing password material:
...
curl --fail \
  ...
  -X PUT \
  -d '{"scopes":["documents:read","search:read","tenant:pro","dataset:*"]}' \
  http://127.0.0.1:5000/auth/users/analyst
...
The accepted lifecycle statuses are =active= and =disabled=. A disabled user
cannot authenticate by password. Disabling a user does not silently mutate
credential records; an operator that needs immediate token invalidation should
also revoke that user's credentials.
