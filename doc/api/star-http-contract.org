...
- Slots: =id=, =client-name=, =method=, =path=, =summary=, =tags=, =authority=, =scopes=, =path-parameters=, =query-parameters=, =request-schema=, =responses=, =idempotency=
...
:END:
#+begin_src lisp
(http-operation-query-parameters (SB-KERNEL:INSTANCE))
#+end_src
...
:END:
#+begin_src lisp
(operation-fbp-node-object (STAR.HTTP.CONTRACT::OPERATION))
#+end_src
