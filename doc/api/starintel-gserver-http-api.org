#+TITLE: API Reference — STARINTEL-GSERVER-HTTP-API
System: =starintel-gserver=
#+OPTIONS: toc:2 num:nil
StarIntel HTTP API.

* Variables and Constants
*** *app*
:PROPERTIES:
:KIND: var
:PACKAGE: STARINTEL-GSERVER-HTTP-API
:END:
The Ningle application instance serving the HTTP API.
*** *default-headers*
:PROPERTIES:
:KIND: var
:PACKAGE: STARINTEL-GSERVER-HTTP-API
:END:
Default response headers applied to API responses.

* Types and Classes
*** bulk-ingest-job
:PROPERTIES:
:KIND: struct
:PACKAGE: STARINTEL-GSERVER-HTTP-API
:END:
One bulk ingest job; terminal jobs retain counts, not request payloads.
- Slots: =id=, =principal=, =documents=, =total=, =correlation-id=, =service-context=, =submitted-at=, =status=, =succeeded=, =failed=, =error-code=

* Conditions
*** http-input-error
:PROPERTIES:
:KIND: condition
:PACKAGE: STARINTEL-GSERVER-HTTP-API
:END:
Signalled for malformed HTTP input payloads.
- Slots: =status=, =code=, =message=, =info=

* Functions
*** bounded-query-integer
:PROPERTIES:
:KIND: fn
:PACKAGE: STARINTEL-GSERVER-HTTP-API
:END:
#+begin_src lisp
(bounded-query-integer (STARINTEL-GSERVER-HTTP-API::PARAMS STARINTEL-GSERVER-HTTP-API::NAME &KEY
 STARINTEL-GSERVER-HTTP-API::DEFAULT (STARINTEL-GSERVER-HTTP-API::MINIMUM 0)
 (STARINTEL-GSERVER-HTTP-API::MAXIMUM
  STARINTEL-GSERVER-HTTP-API::+HTTP-MAX-QUERY-LIMIT+)))
#+end_src
Parse an integer query parameter clamped to sane bounds.
*** bulk-ingest-job-failed
:PROPERTIES:
:KIND: fn
:PACKAGE: STARINTEL-GSERVER-HTTP-API
:END:
#+begin_src lisp
(bulk-ingest-job-failed (SB-KERNEL:INSTANCE))
#+end_src
The =failed= slot of =bulk-ingest-job=.
*** bulk-ingest-job-principal
:PROPERTIES:
:KIND: fn
:PACKAGE: STARINTEL-GSERVER-HTTP-API
:END:
#+begin_src lisp
(bulk-ingest-job-principal (SB-KERNEL:INSTANCE))
#+end_src
The =principal= slot of =bulk-ingest-job=.
*** bulk-ingest-job-service-context
:PROPERTIES:
:KIND: fn
:PACKAGE: STARINTEL-GSERVER-HTTP-API
:END:
#+begin_src lisp
(bulk-ingest-job-service-context (SB-KERNEL:INSTANCE))
#+end_src
The =service-context= slot of =bulk-ingest-job=.
*** bulk-ingest-job-status
:PROPERTIES:
:KIND: fn
:PACKAGE: STARINTEL-GSERVER-HTTP-API
:END:
#+begin_src lisp
(bulk-ingest-job-status (SB-KERNEL:INSTANCE))
#+end_src
The =status= slot of =bulk-ingest-job=.
*** bulk-ingest-job-succeeded
:PROPERTIES:
:KIND: fn
:PACKAGE: STARINTEL-GSERVER-HTTP-API
:END:
#+begin_src lisp
(bulk-ingest-job-succeeded (SB-KERNEL:INSTANCE))
#+end_src
The =succeeded= slot of =bulk-ingest-job=.
*** bulk-request-mode
:PROPERTIES:
:KIND: fn
:PACKAGE: STARINTEL-GSERVER-HTTP-API
:END:
#+begin_src lisp
(bulk-request-mode (STARINTEL-GSERVER-HTTP-API::DOCUMENT-COUNT))
#+end_src
How a bulk request behaves on failure (atomic or best effort).
*** execute-bulk-job
:PROPERTIES:
:KIND: fn
:PACKAGE: STARINTEL-GSERVER-HTTP-API
:END:
#+begin_src lisp
(execute-bulk-job (STARINTEL-GSERVER-HTTP-API::JOB &KEY
 (STARINTEL-GSERVER-HTTP-API::PUBLISH-FN
  #'STARINTEL-GSERVER-HTTP-API:PUBLISH-DOCUMENT)))
#+end_src
Apply every document of a bulk job; returns per-document outcomes.
*** json-array-p
:PROPERTIES:
:KIND: fn
:PACKAGE: STARINTEL-GSERVER-HTTP-API
:END:
#+begin_src lisp
(json-array-p (STARINTEL-GSERVER-HTTP-API::VALUE))
#+end_src
True when the parsed payload is a JSON array.
*** json-object-p
:PROPERTIES:
:KIND: fn
:PACKAGE: STARINTEL-GSERVER-HTTP-API
:END:
#+begin_src lisp
(json-object-p (STARINTEL-GSERVER-HTTP-API::VALUE))
#+end_src
True when the parsed payload is a JSON object.
*** parse-json-octets
:PROPERTIES:
:KIND: fn
:PACKAGE: STARINTEL-GSERVER-HTTP-API
:END:
#+begin_src lisp
(parse-json-octets (STARINTEL-GSERVER-HTTP-API::OCTETS STARINTEL-GSERVER-HTTP-API::CONTENT-TYPE
 &KEY
 (STARINTEL-GSERVER-HTTP-API::MAX-BYTES
  STARINTEL-GSERVER-HTTP-API::+HTTP-MAX-BODY-BYTES+)))
#+end_src
Parse a request body (octets) into a JSON value.
*** publish-document
:PROPERTIES:
:KIND: fn
:PACKAGE: STARINTEL-GSERVER-HTTP-API
:END:
#+begin_src lisp
(publish-document (STARINTEL-GSERVER-HTTP-API::DOCUMENT))
#+end_src
Authorize at the embedded publish boundary before any Rabbit side effect.
*** publish-target-document-unchecked
:PROPERTIES:
:KIND: fn
:PACKAGE: STARINTEL-GSERVER-HTTP-API
:END:
#+begin_src lisp
(publish-target-document-unchecked (STARINTEL-GSERVER-HTTP-API::DOCUMENT))
#+end_src
Publish a legacy target directly to the target compatibility consumer.
*** resolved-server-tenant
:PROPERTIES:
:KIND: fn
:PACKAGE: STARINTEL-GSERVER-HTTP-API
:END:
#+begin_src lisp
(resolved-server-tenant (STARINTEL-GSERVER-HTTP-API::DOCUMENT))
#+end_src
Tenant adopted by a document that carries no tenancy of its own.
*** service-context-properties
:PROPERTIES:
:KIND: fn
:PACKAGE: STARINTEL-GSERVER-HTTP-API
:END:
#+begin_src lisp
(service-context-properties (STARINTEL-GSERVER-HTTP-API::DTYPE NINGLE/CONTEXT:CONTEXT &OPTIONAL
 *CURRENT-AUTHORIZATION-DECISION*))
#+end_src
Build server-owned Rabbit properties. Caller-supplied document fields are ignored.
*** stamp-server-tenant!
:PROPERTIES:
:KIND: fn
:PACKAGE: STARINTEL-GSERVER-HTTP-API
:END:
#+begin_src lisp
(stamp-server-tenant! (STARINTEL-GSERVER-HTTP-API::DOCUMENT))
#+end_src
Inject the resolved tenant_id ahead of MQ publish or persistence.

A document that already declares tenant_id or tenant keeps it untouched.
*** strip-server-tenant-fields
:PROPERTIES:
:KIND: fn
:PACKAGE: STARINTEL-GSERVER-HTTP-API
:END:
#+begin_src lisp
(strip-server-tenant-fields (STARINTEL-GSERVER-HTTP-API::DOCUMENT))
#+end_src
Remove the server-injected tenant_id from an outgoing document.

Accepts either a parsed jsown object or a raw JSON string; strings are
parsed, stripped, and re-serialized so the surrounding handler keeps
operating on the type it started with.
*** strip-server-tenant-from-rows
:PROPERTIES:
:KIND: fn
:PACKAGE: STARINTEL-GSERVER-HTTP-API
:END:
#+begin_src lisp
(strip-server-tenant-from-rows (STARINTEL-GSERVER-HTTP-API::RESPONSE))
#+end_src
Strip tenant_id from every embedded document of a row response.

Covers the FTS stored-fields projection (=fields=), the embedded document
(=doc=), and outbox payloads recorded inside =doc.extensions=.
*** strip-server-tenant-from-search-body
:PROPERTIES:
:KIND: fn
:PACKAGE: STARINTEL-GSERVER-HTTP-API
:END:
#+begin_src lisp
(strip-server-tenant-from-search-body (STARINTEL-GSERVER-HTTP-API::BODY))
#+end_src
Strip tenant_id from docs embedded in a CouchDB FTS response body.

Accepts either a parsed jsown object or a raw JSON string; strings are
parsed, stripped, and re-serialized so the surrounding handler keeps
operating on the type it started with.
*** submit-bulk-ingest-job
:PROPERTIES:
:KIND: fn
:PACKAGE: STARINTEL-GSERVER-HTTP-API
:END:
#+begin_src lisp
(submit-bulk-ingest-job (STARINTEL-GSERVER-HTTP-API::DOCUMENTS STARINTEL-GSERVER-HTTP-API::PRINCIPAL
 &KEY (STARINTEL-GSERVER-HTTP-API::TELL-FN #'SENTO.ACTOR:TELL)
 (STARINTEL-GSERVER-HTTP-API::ENSURE-WORKERS-FN
  #'STARINTEL-GSERVER-HTTP-API::START-BULK-INGEST-WORKERS)))
#+end_src
Reserve bounded queue capacity and enqueue one authenticated job.
*** validate-document-input
:PROPERTIES:
:KIND: fn
:PACKAGE: STARINTEL-GSERVER-HTTP-API
:END:
#+begin_src lisp
(validate-document-input (STARINTEL-GSERVER-HTTP-API::DOCUMENT &KEY
 STARINTEL-GSERVER-HTTP-API::PATH-DTYPE STARINTEL-GSERVER-HTTP-API::INDEX))
#+end_src
Validate a document payload from HTTP input.

Validation is unconditionally strict: every HTTP boundary caller must emit
v0.9-valid documents, including the legacy target adapter, which first
folds its historical envelope with =normalize-legacy-target-document=.

* Generic Functions
*** http-input-error-code
:PROPERTIES:
:KIND: fn
:PACKAGE: STARINTEL-GSERVER-HTTP-API
:END:
#+begin_src lisp
(http-input-error-code (CONDITION))
#+end_src
The =code= slot of =http-input-error=.
*** http-input-error-status
:PROPERTIES:
:KIND: fn
:PACKAGE: STARINTEL-GSERVER-HTTP-API
:END:
#+begin_src lisp
(http-input-error-status (CONDITION))
#+end_src
The =status= slot of =http-input-error=.
