...
:PACKAGE: STARINTEL-GSERVER
:END:
Signalled when an addon fails to load or start.
...
:END:
#+begin_src lisp
(initialize-lease-store (&KEY STARINTEL-GSERVER::FORCE
 (STARINTEL-GSERVER::BACKEND STARINTEL-GSERVER:*LEASE-STORE-BACKEND*)
 (STARINTEL-GSERVER::HOST STARINTEL-GSERVER:*VALKEY-LEASE-HOST*)
 (STARINTEL-GSERVER::PORT STARINTEL-GSERVER:*VALKEY-LEASE-PORT*)
 (STARINTEL-GSERVER::PASSWORD-FILE
  STARINTEL-GSERVER:*VALKEY-LEASE-PASSWORD-FILE*)
 (STARINTEL-GSERVER::METRICS-HOOK
  (STARINTEL-GSERVER:OBSERVABILITY-LEASE-METRICS-HOOK))
 STARINTEL-GSERVER::AUDIT-HOOK))
#+end_src
...
:END:
#+begin_src lisp
(observability-route-template (STARINTEL-GSERVER::ENV))
#+end_src
...
:END:
#+begin_src lisp
(observability-wrapped-server (STARINTEL-GSERVER::APP))
#+end_src
...
:END:
#+begin_src lisp
(parse-tenant-dataset-map (STARINTEL-GSERVER::VALUE))
#+end_src
...
:END:
#+begin_src lisp
(parse-tenant-fallback (STARINTEL-GSERVER::VALUE))
#+end_src
...
:END:
#+begin_src lisp
(shutdown-lease-store (&KEY
 (STARINTEL-GSERVER::DEADLINE
  (+ (STARINTEL-GSERVER::LEASE-STORE-UNIX-MILLISECONDS) 5000))
 (STARINTEL-GSERVER::REQUEST-ID "shutdown-lease-store")))
#+end_src
...
:END:
#+begin_src lisp
(tenant-adaptation-for (STARINTEL-GSERVER::DATASET))
#+end_src
